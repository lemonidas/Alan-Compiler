...
let dummy_elimination block_code = 
...
let immediate_backward_propagation block_code = 
...
let fold_constants block_code =
	...
	let get_constant = function
		...
		|Quad_char(str) -> None (* FIXME: Currently implemented only for ints...*)
		|Quad_entry(ent) ->
			try
